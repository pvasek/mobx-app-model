...
describe('Counter', () => {
    ...
    describe('actions:', () => {
        ...
        const model = getActionsFromModelFactory(createModel);

        it('Should increment counter value', () => {
            const state = { value: 0 };
            model.actions.increment({ state });
        ...
        it('Should reset counter value', () => {
            const state = { value: 5 };
            model.actions.reset({ state });
            assert.equal(state.value, 0);
        });

        it('Should set counter value', () => {
            const state = { value: 0 };
            model.actions.set({ state }, 7);
